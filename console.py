...
# ==========================================
COMPANY_DOMAINS = {
    "Google": ["google.com", "youtube.com", "googleapis.com", "gstatic.com", "googlevideo.com", "googleusercontent.com", "ggpht.com", "blogspot.com", "android.com", "google-analytics.com"],
    "Microsoft": ["microsoft.com", "windows.com", "live.com", "bing.com", "msn.com", "office.com", "office365.com", "azure.com", "skype.com", "hotmail.com", "xbox.com"],
    "Apple": ["apple.com", "icloud.com", "mzstatic.com", "apple-dns.net", "itunes.com", "appstore.com", "me.com", "mac.com", "icloud-content.com"],
    "Amazon": ["amazon.com", "amazonaws.com", "aiv-cdn.net", "amazon-adsystem.com", "media-amazon.com", "primevideo.com", "twitch.tv", "audible.com"]
}
...
class NetworkManager:
    ...
    def update_company_block(self, company, domain_states):
        ...
        if os.path.exists(BLOCKLIST_FILE):
            with open(BLOCKLIST_FILE, "r") as f: existing_lines = f.readlines()
        
        new_lines = []
        skip = False
        for line in existing_lines:
            if f"# {company} Blocklist" in line: skip = True; continue
            if skip and line.strip() == "": skip = False; continue
            if not skip: new_lines.append(line)
        
        if domains_to_block:
            new_lines.append(f"\n# {company} Blocklist\n")
            for domain in domains_to_block: new_lines.append(f"address=/{domain}/0.0.0.0\n")
        
        with open(BLOCKLIST_FILE, "w") as f: f.writelines(new_lines)
    ...
    def update_category_block(self, category, is_enabled):
        ...
        skip = False
        for line in existing_lines:
            if f"# {category} Blocklist" in line: skip = True; continue
            if skip and line.strip() == "": skip = False; continue
            if not skip: new_lines.append(line)
        
        if is_enabled:
            new_lines.append(f"\n# {category} Blocklist\n")
            for domain in domains: new_lines.append(f"address=/{domain}/0.0.0.0\n")
...
class PiNetMasterApp(ctk.CTk):
    ...
    def _get_interfaces(self):
        res = subprocess.run("ip -o link show | awk -F': ' '{print $2}' | tr -d '@' | grep -v '^lo$'", shell=True, capture_output=True, text=True)
        if res.returncode == 0:
            return sorted([iface.strip() for iface in res.stdout.split('\n') if iface.strip()])
        return ["eth0", "wlan0"]

    def _get_vpn_interfaces(self):
        res = subprocess.run("ip -o link show | awk -F': ' '{print $2}' | tr -d '@' | grep -E '^(tun|wg|vpn)'", shell=True, capture_output=True, text=True)
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(self.sidebar, text="Pi NetMaster", font=ctk.CTkFont(size=22, weight="bold"), text_color="#0078D4").pack(pady=30, padx=20)
        
        self.btn_bridge = ctk.CTkButton(self.sidebar, text="Network Bridge", command=self.show_bridge, fg_color="#252525", hover_color="#333333", anchor="w", height=40)
        self.btn_bridge.pack(pady=5, padx=20, fill="x")
        
        self.btn_settings = ctk.CTkButton(self.sidebar, text="Settings", command=self.show_settings, fg_color="#252525", hover_color="#333333", anchor="w", height=40)
    ...
    def show_bridge(self):
        ...
        self.btn_fix_engine = ctk.CTkButton(status_frame, text="Fix Router Engine", command=self._fix_engine_click, fg_color="#D32F2F", hover_color="#B71C1C")
        ...
        ctk.CTkLabel(ds_frame, text="2. Downstream Interfaces (Client Output)", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        ctk.CTkLabel(ds_frame, text="Select one or more interfaces. Only ONE Wi-Fi interface can be selected.", text_color="#888888", font=ctk.CTkFont(size=12)).pack(pady=(0, 10))
        ...
        for iface in self.interfaces:
            ...
            ctk.CTkCheckBox(ds_scroll, text=iface, variable=var, 
                            ...
                            font=ctk.CTkFont(size=14)).pack(anchor="w", pady=2)
        ...
        self.btn_share_on = ctk.CTkButton(btn_frame, text="Turn ON Sharing", command=self.toggle_bridge_on, fg_color="#28A745", hover_color="#218838", width=150)
        self.btn_share_on.pack(side="left", padx=15)
        self.btn_share_off = ctk.CTkButton(btn_frame, text="Turn OFF Sharing", command=self.toggle_bridge_off, fg_color="#DC3545", hover_color="#C82333", width=150)
    ...
    def _update_downstream_config_ui(self):
        ...
        if not selected_ds:
            ctk.CTkLabel(self.downstream_config_frame, text="No downstream interfaces selected.", text_color="#888888").pack(pady=20)
        ...
        if has_eth:
            ctk.CTkLabel(self.downstream_config_frame, text="Ethernet DHCP Configuration", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
            ctk.CTkLabel(self.downstream_config_frame, text="Will automatically assign unique subnets to each selected Ethernet port.", 
    ...
    def toggle_bridge_on(self):
        ...
        for iface in selected_ds:
            ...
            if mac_mode != "None":
                ...
                if mac_mode == "Randomize": cmds.append(f"macchanger -r {iface}")
                else:
        ...
        for cmd in cmds: self.net_mgr._run_cmd(cmd)
    ...
    def _fix_engine_click(self):
        ...
        self._update_engine_status_ui()
        if success: messagebox.showinfo("Success", "Router Engine Fixed!")
        else: messagebox.showerror("Error", "Failed to initialize engine.")
    ...
    def show_settings(self):
        ...
        ctk.CTkLabel(scroll_frame, text="Filter Lists & Settings", font=ctk.CTkFont(size=24, weight="bold"), anchor="w").pack(pady=20, padx=20, fill="x")
        ctk.CTkLabel(scroll_frame, text="Big Tech Filtering (Customizable)", font=ctk.CTkFont(size=16, weight="bold"), anchor="w").pack(pady=(20, 10), padx=20, fill="x")
        ...
        ctk.CTkLabel(scroll_frame, text="Content Filtering", font=ctk.CTkFont(size=16, weight="bold"), anchor="w").pack(pady=(20, 10), padx=20, fill="x")
